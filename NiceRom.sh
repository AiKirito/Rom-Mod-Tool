...
modify_animation() {
    ...
    local apksigner_path="$(dirname "$0")/bin/all/apksigner/apksigner.jar"
    local key_path="$(dirname "$0")/bin/all/apksigner/my_signer.pk8"
    local cert_path="$(dirname "$0")/bin/all/apksigner/my_signer.x509.pem"
    charge_resource_dir="$(dirname "$0")/bin/xiaomi/animations/charge"
    fingerprint_animation_dir="$(dirname "$0")/bin/xiaomi/animations/fingerprint"

    while IFS= read -r -d '' apk_path; do
        if [[ $apk_path == *"product/overlay"* || $apk_path == *"product_a/overlay"* ]]; then
            dir_path=$(dirname "$apk_path")
            output_dir="$dir_path/DevicesOverlay"
            java -jar $apktool_path d -f "$apk_path" -o "$output_dir"

            rm -f "$output_dir/res/values/public.xml"

            if [ -d "$output_dir/res/drawable-440dpi" ]; then
		find "$output_dir/res/drawable-440dpi/" -type f -exec rm -f {} \;
                cp -r $fingerprint_animation_dir/* "$output_dir/res/drawable-440dpi/"
            fi

            while IFS= read -r -d '' file; do
                base_file=$(basename "$file")
                if [ -f "$charge_resource_dir/$base_file" ]; then
                    cp "$charge_resource_dir/$base_file" "$file"
                fi
            done < <(find "$output_dir" -type f -print0)

            java -jar $apktool_path b -f  "$output_dir" -o "$apk_path"
            rm -rf "$output_dir"

            java -jar $apksigner_path sign --key $key_path --cert $cert_path --v1-signing-enabled true --v4-signing-enabled false $apk_path
        fi
    done < <(find "$onepath" -type f -name "DevicesOverlay.apk" -print0)
}
...
update_build_props() {
  declare -A lines_to_add=(
    ...
    [system_a]="persist.sys.background_blur_supported=true persist.sys.background_blur_status_default=true persist.sys.background_blur_mode=0 persist.sys.background_blur_version=2 debug.game.video.speed=1 debug.game.video.support=1"
    [vendor_a]="ro.vendor.se.type=HCE,UICC,eSE ro.vendor.audio.sfx.scenario=true"
    [product_a]="persist.sys.miui_animator_sched.sched_threads=2 persist.vendor.display.miui.composer_boost=4-7"
)
  while IFS= read -r -d '' dir; do
    local type="${dir##*/}"
    local build_prop_path="$dir/build.prop"
    if [[ "$type" == "product" || "$type" == "product_a" ]]; then
      build_prop_path="$dir/etc/build.prop"
    fi
    if { [[ "$type" == "system" && -d "$dir/framework" ]] || 
         [[ "$type" == "vendor" && -d "$dir/etc" ]] ||
         [[ "$type" == "system_a" && -d "$dir/framework" ]] ||
         [[ "$type" == "vendor_a" && -d "$dir/etc" ]] ||
         [[ "$type" == "product" && -d "$dir/etc" ]] ||
         [[ "$type" == "product_a" && -d "$dir/etc" ]]; } && [[ -f "$build_prop_path" ]]; then
  ...
  done < <(find "$onepath" -type d \( -name 'system' -o -name 'vendor' -o -name 'system_a' -o -name 'vendor_a' -o -name 'product' -o -name 'product_a' \) -print0)
}
...
prevent_theme_reversion() {
  ...
  done < <(find "$onepath" \( -type d -name 'system_ext' -o -name 'system_ext_a' \) -print0)
}
...
copy_dir_xiaomi() {
    ...
    while IFS= read -r -d '' dir; do
        if [ -d "$dir/bin" ] && [ -d "$dir/media" ] && [ -d "$dir/overlay" ]; then
    ...
    done < <(find "$onepath" -type d \( -name "product" -o -name "product_a" \) -print0)
}
...
replace_files_xiaomi() {
    ...
    done < <(find "$dst_dir" -type f -o -type d -print0)
...
while true; do
    ...
    select brand in "${rom_brands[@]}"; do
        case $brand in
            "HyperOS")
                echo "已选择 HyperOS"
                options_order=("删除小爱翻译" "删除小爱语音组件" "删除小爱通话" "删除互联互通服务 ROOT 验证" "删除浏览器" "删除音乐" "删除视频" "删除钱包" "删除广告与分析组件" "删除 Joyose 云控" "删除自带输入法" "删除传送门" "删除智能助理" "删除搜索功能" "删除悬浮球" "删除应用商店" "删除服务与反馈" "删除系统更新" "删除家人守护" "删除下载管理器" "删除可能不需要的应用" "删除所有" "HyperOS 替换" "HyperOS 添加" "HyperOS 动画调整" "禁用未签名验证" "调用原生安装器" "禁止主题还原" "添加新特性" "关键性 deodex" "Deodex" "退出")
                declare -A options
                options=(
                    ["删除小爱翻译"]="AiAsstVision*"
            ...
            "OneUI")
                echo "已选择 OneUI"
options_order=("删除三星浏览器组件" "删除开机验证" "删除 Rec 恢复为官方" "解码 csc" "编码 csc" "关键性 deodex" "Deodex" "删除所有" "退出")
declare -A options
                options=(
